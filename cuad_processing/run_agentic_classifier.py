...
"""
Agentic Contract Classifier (LangGraph)

This module uses AWS Bedrock's OSS120 model with LangGraph's reflection pattern
to automatically label and categorize contract text across the 41 CUAD categories.

The agentic approach uses a three-agent reflection system:
- Classifier: Initial classification
- Critic: Reviews and provides feedback
- Arbitrator: Makes final decisions

This is slower but more accurate than standard classification.

ZERO ARGUMENTS REQUIRED - Just run: python run_agentic_classifier.py
"""
...
def main():
    """Label contracts using AWS Bedrock OSS120 model with LangGraph reflection pattern.
    
    All paths and settings are hardcoded - no arguments needed.
    Simply run: python run_agentic_classifier.py
    
    This uses the agentic (LangGraph) approach with three-agent reflection for higher accuracy.
    """
    import os
    
    # Hardcoded configuration for AGENTIC CLASSIFIER
    config_path = './cuad_processing/config.yaml'
    input_file = None  # Not used when processing output folder
    input_csv = None  # Not used when processing output folder
    output_dir = './cuad_processing/output'
    output_file = None  # Auto-generated (will be labeled_contracts_langgraph.csv)
    process_output_folder = True  # Always process output folder
    threads = 200  # Parallel threads (not used for LangGraph - sequential only)
    no_resume = False  # Enable resume by default
    parallel = False  # LangGraph uses sequential processing for reflection
    use_langgraph = True  # ✓ AGENTIC MODE ENABLED - LangGraph reflection pattern
    
    # Initialize labeler
    labeler = OSS120ContractLabeler(config_path=config_path)
    ...
    if process_output_folder:
        ...
        # Set default output path if not specified (LangGraph-specific)
        if not output_path:
            output_path = str(output_dir / 'labeled_contracts_langgraph.csv')
    
    elif input_file_csv:
        # Process specific CSV file
        print(f"Loading contracts from CSV: {input_file_csv}")
        contracts = load_contracts_from_csv(input_file_csv)
        print(f"Loaded {len(contracts)} contracts")
        
        if not output_path:
            input_path = Path(input_file_csv)
    ...
    if use_langgraph:
        ...
        print(f"Parallel processing: {parallel}, Threads: {threads}, Resume: {not no_resume}")
